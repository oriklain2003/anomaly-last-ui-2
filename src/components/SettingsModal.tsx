...
import { X, RotateCcw, Moon, Sun, ChevronDown, Globe } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
    ...
    const { language, setLanguage, isHebrew, analystLanguageSetting, setAnalystLanguageSetting } = useLanguage();
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className={`bg-surface border border-white/10 rounded-xl p-6 w-[480px] max-h-[90vh] overflow-hidden shadow-2xl relative flex flex-col ${isHebrew ? 'text-right' : ''}`} dir={isHebrew ? 'rtl' : 'ltr'}>
                ...
                <div className="space-y-6 overflow-y-auto flex-1 pe-1 ps-1">
                    
                    {/* Language Settings */}
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-white/80">{t('settings.language')}</label>
                    ...
                    {/* Visual Analyst Language Settings */}
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-white/80">{t('settings.analystLanguage')}</label>
                        <div className="flex gap-2">
                            <button
                                onClick={() => setAnalystLanguageSetting('auto')}
                                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all border ${
                                    analystLanguageSetting === 'auto'
                                        ? "bg-primary text-white border-primary"
                                        : "bg-white/5 text-white/60 border-white/10 hover:bg-white/10 hover:text-white"
                                }`}
                            >
                            ...
                            <button
                                onClick={() => setAnalystLanguageSetting('en')}
                                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all border ${
                                    analystLanguageSetting === 'en'
                                        ? "bg-primary text-white border-primary"
                                        : "bg-white/5 text-white/60 border-white/10 hover:bg-white/10 hover:text-white"
                                }`}
                            >
                            ...
                            <button
                                onClick={() => setAnalystLanguageSetting('he')}
                                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all border ${
                                    analystLanguageSetting === 'he'
                                        ? "bg-primary text-white border-primary"
                                        : "bg-white/5 text-white/60 border-white/10 hover:bg-white/10 hover:text-white"
                                }`}
                            >
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-white/80">{t('settings.primaryColor')}</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-white/80">{t('settings.secondaryColor')}</label>
                        <div className="flex gap-3">
                            <input
                                type="color"
                                value={theme.surface}
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-white/80">{t('settings.accentColor')}</label>
                        <div className="flex gap-3">
                            <input
                                type="color"
                                value={theme.accent}
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {darkPresets.map(([key, preset]) => (
                                            <button
                                                ...
                                                className="group flex items-center gap-2.5 p-2.5 rounded-lg border border-white/10 bg-background-dark/50 hover:border-primary/60 hover:bg-white/5 transition-all text-start"
                                            >
